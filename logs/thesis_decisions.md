...
## 2026-04-20 — Pre-drafting drug-level diagnostics: 2009-2013 CS NDA list and top-sponsor table

Built `09_cs_nda_drug_investigation.do` to address two loose ends before manuscript drafting. (1) The Poisson event-study DD in 08 shows large positive IRR spikes for `is_cs#event_shifted` interactions in 2010 (IRR≈6.3), 2011 (IRR≈9.6), and 2017 (IRR≈4.1) — all 18-25 years after PDUFA. Hypothesis: these reflect the late-2000s extended-release and abuse-deterrent branded opioid wave (Opana ER, Exalgo, Zohydro ER, OxyContin reformulation), not a delayed PDUFA composition effect. The new file extracts the specific drug, sponsor, and DEA schedule for every CS NDA approved 2009-2013 so the discussion section can name the drugs and sponsors driving the spike, supporting the manuscript's argument that the rate-based "effect" is a post-policy industry product cycle rather than a PDUFA-induced composition shift. Validation: 43 CS NDAs in the 2009-2013 window; 2010 (11 drugs) and 2011 (16 drugs) are the peak years; 58% are Schedule II; `drugname_list` is fully populated (EDLUAR, ONSOLIS, EMBEDA, etc.). Annual counts and brand names confirm the opioid-era hypothesis. (2) Replaced the noisy annual CS-NDA top-10 sponsor share figure (`pdufa_cs_nda_sponsor_concentration_top10.png`) with a static table of the top sponsors of CS NDAs over the full 1970-2025 window (`cs_nda_top_sponsors_1970_2025.csv`, 127 sponsors ranked). Annual CS-NDA counts are often single-digit, making the time-series figure jump between 0% and 100% in ways that obscure rather than illuminate. Static table will be the paper exhibit for CS-NDA market structure; top sponsors include HIKMA and UCB INC (9 each, ~4% share), SPECGX LLC and PURDUE PHARMA LP (7 each, ~3%), INDIVIOR and UPJOHN (5 each). The PNG figure is left on disk but is NOT for manuscript inclusion. Added `09` to `00_master.do`.

---

## 2026-04-20 — Added rate-vs-count diagnostics, NDA sponsor concentration, GDUFA reference lines

Four additions across three files. (1) `08_subsample_alt_specs.do` Part 6: rate-vs-count two-panel figure (`pdufa_nda_rate_vs_count.png`) showing NDA CS share alongside raw CS and non-CS count series; trend-control Poisson sensitivity with linear year trend and group-specific trends (models `m_nda_d3_trend`, `m_nda_d3_grouptrend`); timing diagnostic Poisson DD with IRRs at selected event-time bins; results exported to `pdufa_nda_diagnostics.csv`. Rationale: NDA-only Poisson (IRR ≈ 1.46, p = 0.06) and linear-levels DD (coef = −23.75, p < 0.001) are simultaneously correct — rate can rise while absolute non-CS counts grow faster in absolute terms. The diagnostics allow the paper to document whether the rate effect survives trend controls. (2) `07_alt_specs_gdufa.do` Part 7: NDA sponsor concentration figures (all-NDA and CS-NDA HHI + top-10 share, 1970–2025, PDUFA reference lines), exported to `pdufa_nda_sponsor_concentration_top10.png`, `pdufa_nda_sponsor_concentration_hhi.png`, `pdufa_cs_nda_sponsor_concentration_top10.png`, and `pdufa_nda_concentration_comparison.csv`. (3) `07_alt_specs_gdufa.do` Part 8: GDUFA rate-vs-count two-panel figure (`gdufa_anda_rate_vs_count.png`), trend-control sensitivity (models `m_gd3_trend`, `m_gd3_grouptrend`), and GDUFA event-study timing diagnostic; exported to `gdufa_diagnostics.csv`. (4) `02_descriptive_figures.do`: added `xline(2012, lpattern(dash) lcolor(olive) lwidth(medthin))` and updated figure notes to "Dashed lines: 1984 (Hatch-Waxman), 1992 (PDUFA), 2012 (GDUFA)." for Figures 1–5, 7, and 8 (all year-indexed series; bar chart Figure 6 excluded). Future extension: when GDUFA II (2017) and III (2022) data accumulate, the user-fee-schedule extension should use the same `gdufa_stacked_dd.dta` design with additional `post_gdufa2`/`post_gdufa3` indicators.

---

## 2026-04-20 — Applied `r(110)` and `r(198)` fixes to `07_alt_specs_gdufa.do`

Two bugs were present. First, `gen trend_post_gdufa` crashed with `r(110); already defined` because `05_gdufa_analysis.do` saves that variable into `gdufa_anda_annual.dta`; fixed with `capture drop` guards before both `gen` statements in Part 1. Second, the sponsor concentration block (Part 5) crashed with `r(198); invalid name` at `sort approval_year (-n_sponsor_year)` — Stata's `sort` does not accept a `-` prefix for descending order; that is `gsort` syntax. Fixed by replacing all three occurrences with `gsort approval_year -n_sponsor_year`. Additionally corrected the `twoway rarea` shading: the original code used `gdufa_transition` (a 0/1 dummy) as both bounds of `rarea`, which draws a sliver at y=0–1 rather than a full-height band. Replaced with proper `shade_lo`/`shade_hi` variables scoped via `if gdufa_transition == 1`, matching the pattern used in `05_gdufa_analysis.do`. Also removed a redundant dead-code preserve block that created an unused `hhi_all` tempfile. File now runs end-to-end; all sponsor concentration figures and the comparison CSV export correctly.
