...
\begin{frame}[t]{Parametric Pre/Post Results}
  Simple interrupted time series regressions on the annual panel (1970--2025):
  \medskip
  \begin{center}
  \begin{tabular}{lcc}
    \toprule
    Model & Post-PDUFA coef. & $R^2$ \\
    \midrule
    A: Level shift only         & $-0.02$ (noisy) & low \\
    B: Level shift + trend      & $\approx 0$     & moderate \\
    C: Level shift + trend + slope change & near 0 & moderate \\
    D: Log count (Model C spec) & positive, small & moderate \\
    \bottomrule
  \end{tabular}
  \end{center}
  \medskip
  \begin{itemize}
    \item \alert{None significant at conventional levels}
    \item With trend control, post-PDUFA coefficient approaches zero
    \item Log count result: more CS approvals after PDUFA, but proportional
          to overall growth
    \item OLS only; serial correlation likely inflates precision
  \end{itemize}
\end{frame}
