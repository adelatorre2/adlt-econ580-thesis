...
Five data limitations bear on the interpretation of the results. First, the DEA linkage reflects current scheduling status rather than historical status at the time of approval. The direction of any resulting bias is ambiguous, but the conservative-match definition concentrates the linkage on substances that have been continuously scheduled for decades, where the misclassification risk is small. Second, the Drugs@FDA database is approval-centered: failed and withdrawn applications are not observed, which limits the ability to study the intensive margin of manufacturer drug-development decisions. Third, the ingredient-level linkage cannot fully resolve combination products that mix controlled and non-controlled ingredients; the variable \texttt{is\_multi\_ingredient} flags these cases, which account for roughly 8\% of CS approvals. Fourth, FDA review-priority codes are unstable over time and are not used as a primary outcome. Fifth, the annual outcome series is a single national time series with no cross-sectional control unit, so the empirical strategy in Section~\ref{sec:strategy} relies on time-series and group-comparison designs rather than on cross-sectional difference-in-differences identification. The estimates that follow should be read as policy-diagnostic reduced-form evidence rather than as point-identified causal effects.
